// 定义瀑布流算法函数
function fall(info) {
  let img_list = document.querySelectorAll(info.el)
  for (let i=0;i<img_list.length;i++){
    let img_obj = $(img_list[i]).find('.img_box img')[0]
    if (img_obj.complete){
      fall_calculation()
    }
    else {
  ...
  function fall_calculation () {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (i < column) {
        ...
        // 保存当前元素的高度。
        this_height = gap + $(items[i]).height()
        ...
        this_height = (minHeight + gap * 2) + $(items[i]).height()
      }
      max_height = Math.max(max_height,this_height)
    }
    document.querySelectorAll(info.parent_el)[0].style.height = max_height + 'px';
  })
  }
}
